...
data "oci_identity_compartments" "COMPARTMENTS" {
  ...
  filter {
    name   = "name"
    values = [var.adb_instance_compartment_name]
  }
}
...
data "oci_identity_compartments" "NWCOMPARTMENTS" {
  ...
  filter {
    name   = "name"
    values = [var.adb_network_compartment_name]
  }
}
...
locals {
 ...
 # Compartment OCID Local Accessor 
  ...
  nsg_id = lookup(data.oci_core_network_security_groups.NSG.network_security_groups[0], "id")
}